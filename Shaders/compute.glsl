...
float BC(int valId, int i, int j, int iOffset, int jOffset) {
    ...
    if (objectFlag) {
        ...
        switch (valId) {
            ...
            //e
            case 3:
                return fields[newIndex].E - 0.5 * (fields[newIndex].u*fields[newIndex].u + fields[newIndex].v*fields[newIndex].v) + 0.5 * 50*50.0;
            //S 
            case 4:
                return fields[newIndex].S;
        }
...
void main() {
    ...
    vec3 sEdVIEW = vec3(sqrt(outFields[index].u*outFields[index].u+outFields[index].v*outFields[index].v)/75.0,outFields[index].E / 50.0,outFields[index].d/2.5);
